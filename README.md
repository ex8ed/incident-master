...
[**Его необходимо создать и заполнить.**](https://pastebin.com/EN2MEecd); (ссылка на pastebin с содержимым)

docker-compose.yml сделан так, что сам поднимает рядом с сервисом контейнер с базой данных, поэтому этот шаг
можно считать условным, но база данных просто не поднимется без этих переменных в окружении.
...
### Получение индицента по id (GET) /incidents/{id}
